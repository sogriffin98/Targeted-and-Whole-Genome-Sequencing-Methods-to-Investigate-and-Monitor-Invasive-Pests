...
setwd("B:/combined_run_data/PCA/Original_WORKED/UK_Only_1pernest/vcf")
...
PCA_file_covMat <- read.table("UK_pca_results.eigenvec", header=FALSE) 
# import the .cov file
eigenvals <- read.table("UK_pca_results.eigenval", header=FALSE)
...
meta<- read.csv("sample_ID_UK_1pernest.csv", header=FALSE)
head(meta)
pop_assings <- meta[,2]
