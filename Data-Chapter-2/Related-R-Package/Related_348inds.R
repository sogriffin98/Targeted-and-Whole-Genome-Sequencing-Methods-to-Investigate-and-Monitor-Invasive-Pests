...
# Load genotype data
genotype_data <- read.table("UK_ALL_Relatedness_convert.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
...
write.table(data_df, file = "genind_dataframe.txt", sep = "\t", row.names = TRUE, col.names = FALSE)
...
# Select only necessary columns (assuming 122 loci)
# 122 loci × 2 alleles + 1 ID column = 245 columns
related_input_trim <- related_input[, 1:(2 * 106 + 1)]
...
write.table(related_input, "related_input.txt", sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)

# Now read into related package:
related_data <- readgenotypedata("related_input.txt")

######## Compare estimators
sink("estimator_output_SNPs_1000iterations.txt")
compareestimators(related_data, 300)
sink()

# Calculate all 5 estimators for your data
results <- coancestry(
  related_data,
  wang = TRUE,
  lynchli = TRUE,
...
sink("estimator_output_SNPs_all5.txt")
print(results$relatedness)
sink()


# Calculate relatedness values
genotypes <- read.table("related_input.txt", header = FALSE)
relatedness_output <- coancestry(genotypes, error.rates = 0.05, lynchrd = 1)
write.table(relatedness_output$relatedness, "relatedness_output_terminal_1000iterations.txt", sep = ",", row.names = TRUE, col.names = TRUE)

# Import data
related_results <- read.table("relatedness_output_terminal_1000iterations.txt", header = TRUE, sep = ",", row.names = 1)
...
pdf("relatedness_histogram_1000iterations.pdf", width = 8, height = 6) 
ggplot(relatedness_output$relatedness, aes(x = lynchrd)) +
  geom_histogram(binwidth = 0.025, colour = "black", fill = "steelblue") +
  scale_x_continuous(expand = c(0, 0), name = "Lynch & Ritland Relatedness") +
  scale_y_continuous(expand = c(0, 0), name = "Density") +
...
write.table(relatedness_output$relatedness,"relatedness_1000iterations.txt",sep=",",row.names = TRUE, col.names = TRUE)

# Simulate related individuals based on allele frequencies from our data:
sim <- familysim(related_data$freqs, 1000)
sim_output  <- coancestry(sim , quellergt = 1)
simrel  <- cleanuprvals(sim_output$relatedness , 1000)
relvalues  <- simrel[, 10]
label1  <- rep("PO", 1000)
label2  <- rep("Full", 1000)
label3  <- rep("Half", 1000)
label4  <- rep("Unrelated", 1000)
...
# Write simulated dataset to a text document
write.table(sim,"simulated_1000iterations.txt",sep=",",row.names = TRUE, col.names = TRUE)
